...
public class Player_move {
	...
	private static List<Card> chooseCardsToDiscard(Player player) {
		...
		for (int i : indexs) {
			toDiscard.add(player.myDeck.getCardsInHand().get(i));
		}
		return toDiscard;
	}
	...
	public static int executeRemoveBlock(Template board, Player player, int NextToBlockade, Blockade block,
			int residualPower) {
	...
	public static void singleMove2(Template board, Player player, int index) {
		...
		while (residualPower > 0) {
			...
			if (movingToInt[0] == -100) {
				...
				boolean discardMovable = InputHelper
						.getYesNoInput("Are you sure to discard this " + selectedAsset + "?");
				if (discardMovable) {
					residualPower = 0;
					break;
				} else {
					System.out.println(
							"You can discard the " + selectedAsset + " or make a movement with the remaining power.");
					continue;
				}
			}
				...
				if (NextToBlockade == -1) {
					System.out.println("You're not next to a blockade.");
				} else {
					...
					if (Util.getColorFromString(selectedAsset.getCardType().toString()).equals(block.getColor())
						...
						if (residualPower >= block.getPoints()) {
							...
							System.out
									.println("Your " + selectedAsset + " does not have enough power to move blockade.");
						}
						...
						if (!(selectedAsset instanceof Card)) {
							System.out.println("Choose a card, not a Token.");
							break;
						}
						List<Card> toDiscard = new ArrayList<>();
				...
				if (Util.getColorFromString(selectedAsset.getCardType().toString()).equals(movingTo.getColor())
					...
					if (player.getCurrentRow() != movingTo.getRow() || player.getCurrentCol() != movingTo.getCol()) {
						System.out.println("You need to discard " + movingTo.getPoints()
								+ " card to get here. Choose all the cards.");
						toDiscard = chooseCardsToDiscard(player);
					...
					System.out.println(
							"Color of " + selectedAsset + " does not match tile color." + selectedAsset.getCardType());
				}
			}
		}
		if (residualPower == 0) {
