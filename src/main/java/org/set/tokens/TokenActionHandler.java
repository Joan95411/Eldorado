...
public class TokenActionHandler {
    ...
    public void doAction(Token token, Player player) {
        ...
        if (caveTokenType == CaveTokenType.Machete) {
        ...
        } else if (caveTokenType == CaveTokenType.Coin) {
            ...
            // Play machete, coin, or paddle tokens to move onto same-colored spaces.
            // Alternatively, you can use coin tokens to buy a card.
        } else if (caveTokenType == CaveTokenType.Draw) {
            drawAction(player);
        } else if (caveTokenType == CaveTokenType.Remove) {
            removeAction(player);
        } else if (caveTokenType == CaveTokenType.Replace) {
            replaceAction(player);
        } else if (caveTokenType == CaveTokenType.ImmediatePlay) {
            immediatePlayAction(player);
        } else if (caveTokenType == CaveTokenType.PassThrough) {
            passThroughAction(player);
        } else if (caveTokenType == CaveTokenType.Adjacent) {
            adjacentAction(player);
        } else if (caveTokenType == CaveTokenType.Symbol) {
            symbolAction(player);
        }
    }
